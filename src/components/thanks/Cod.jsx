...
function Cod({checkoutId, totalPrice, title, shippingCost, productPrice}) {
  return (
    <div className='text-center'>
      ...
      <table className='mx-auto mt-5 w-64'>
        <tbody>
          <tr>
            <td>Harga produk</td>
            <td>:</td>
            <td>{currencyFormat(productPrice)}</td>
          </tr>
          <tr>
            <td>Ongkos Kirim</td>
            <td>:</td>
            <td>{currencyFormat(shippingCost)}</td>
          </tr>
          <tr>
      ...
      <p className="font-bold text-green-500 text-2xl sm:text-3xl my-5">{currencyFormat(totalPrice)}</p>
      <p className="text-sm sm:text-base mb-2">Jangan lupa untuk mengkonfirmasi pesanan Anda ke admin kami.</p>
      <Link href={`https://api.whatsapp.com/send?phone=${process.env.NEXT_PUBLIC_PHONE}&text=Kak%20saya%20mau%20mengkonfirmasi%20pesanan%20COD%20produk%20${title}.%20Order%20id:%20${checkoutId}.%20Silahkan%20diproses%20ya`} target='_blank'>
